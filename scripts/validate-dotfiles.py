...
SOURCE = ROOT / "home"
REQUIRED = {".cargo/config.toml", ".zshenv", ".zprofile", ".zshrc",
            ".config/mise/config.toml", ".config/uv/uv.toml", ".ssh/config"}
...
def validate(arch, work):
    with tempfile.TemporaryDirectory(prefix="dotfiles-validation-") as temporary:
        ...
        env = {**os.environ, "HOME": temporary, "CI": "true",
                "XDG_CONFIG_HOME": str(home / ".config"),
                "XDG_CACHE_HOME": str(home / ".cache"),
                "XDG_DATA_HOME": str(home / ".local/share"),
                "XDG_STATE_HOME": str(home / ".local/state")}
        command = ["chezmoi", f"--source={ROOT}", f"--destination={home}",
                    f"--config={config}", f"--cache={home / 'cache'}",
                    f"--persistent-state={home / 'state.boltdb'}", "--no-tty",
                    "--refresh-externals=never", "--override-data",
        ...
        for index, source in enumerate(files):
            ...
            if source.name.startswith("dot_z"):
                ...
                subprocess.run(["shellcheck", "--shell=bash", str(target)],
                                env=env, check=True)
