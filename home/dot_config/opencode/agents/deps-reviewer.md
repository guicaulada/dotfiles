---
description: Reviews a single dependency bump PR with focus on compatibility and security. Spawned by /review-deps orchestrator to enable parallel dependency reviews. Returns structured report for aggregation.
mode: subagent
color: warning
tools:
  edit: false
...
**Security** — Search for CVEs and advisories for both old and new versions. Check if the update itself is a security fix (look for "security", "CVE", "vulnerability" in PR body/changelog). Use WebSearch for `"{package}" {version} security advisory`. Limit to 2 search queries per dependency to avoid excessive latency.
